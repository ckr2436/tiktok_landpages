...
<?php endif; ?>
  ...
  (function () {
    ...
    function buildShopPdpUrl() {
      var raw = String(tiktok.raw_pdp_url || '');
      if (!raw) { return ''; }
      try {
        ...
        var query = [];
        if (chainValue) {
          try { chainKey = encodeURIComponent(JSON.stringify(JSON.parse(chainValue))); }
          catch (_) { chainKey = encodeURIComponent(chainValue); }
          query.push('chain_key=' + chainKey);
        }
        query.push('source=anchor');
        return parsed.protocol + '//' + parsed.host + path + '?' + query.join('&');
      } catch (_) {
        return raw;
      }
    }
